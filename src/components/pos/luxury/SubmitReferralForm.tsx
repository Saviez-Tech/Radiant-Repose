...
import { submitReferral, fetchMyWorkerProfile } from "@/actions/loyalty.server";
...
export default function SubmitReferralForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result.success) {
      toast.success(`Referral recorded! Referrer earned ${result.data?.points_awarded_to_referrer || 0} pts.`);
      form.reset();
    } else {
    ...
    <div className="max-w-md w-full mx-auto p-6 bg-white">
      <div className="mb-6">
        <h2 className="text-lg font-medium text-primary-deepBlack flex items-center gap-2 mb-3">
          <Users className="w-5 h-5 text-primary-darkRed" />
          Submit Referral
        </h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <input
            ...
            placeholder="New Customer Phone Number (e.g. 08012345678)"
          />
        </div>
        ...
        <div>
          <input
            ...
            required
            className="w-full text-sm px-4 py-4 border border-gray-300 rounded-lg bg-gray-50 placeholder-primary-dark_gray/50 focus:outline-none focus:ring-2 focus:ring-primary-darkRed focus:border-transparent transition-all duration-200"
            placeholder="Referrer ID (e.g. RADIANT-9X42L1)"
          />
        </div>
