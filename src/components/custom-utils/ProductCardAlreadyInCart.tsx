...
export default function ProductCardAlreadyInCart({
  product,
}: {
  product: Product;
}) {
  ...
  const quantity =
    items.find((item) => item.product === product.id)?.quantity || 0;
          ...
          <p className="absolute bottom-4 opacity-90 left-1 z-10 rounded-3xl flex items-center gap-1 p-2 bg-primary-base_color1 text-[10px] md:text-xs font-medium text-brand-primary-light_black">
            ...
            <span className="flex items-center">
              <Icon
                ...
                width="11"
                height="11"
                className="-translate-y-[1px] text-primary-yellow"
              />
      ...
      <div className="p-2">
        ...
        <div className="flex justify-between gap-1 flex-wrap items-center my-2">
          <div className="flex items-center">
            <DollarSquare />
            <span
              className={`${dm_mono.className} text-primary-dark_gray text-xs font-light ml-1`}
            >
              {formatNaira(product.price, false, true)}
            </span>
          </div>
          <span className="text-primary-dark_gray text-[10px] md:text-xs">
        ...
        <div className="flex justify-between items-center gap-2 mt-1">
          ...
          <span
            className={`${dm_mono.className} text-primary-dark_gray text-xs font-medium`}
          >
