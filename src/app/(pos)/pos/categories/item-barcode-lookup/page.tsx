...
import { addScannedItem, removeSearchValue } from "@/lib/redux/slices/posFlowSlice"
...
export default function ItemBarCodeManualLookupPage() {
    ...
    const handleFetchProductByBarcode = async () => {
        setIsLoading(true)
        setItems([])
        const { products, errorMessage, status } = await fetchProductAction(searchValue!)
    ...
    const handleItemRemove = useCallback((barcode: string) => {
        if (items.length === 1){
    ...
    useEffect(() => {
        ...
        setItems((prev) => {
            const updatedItems = prev.filter(
              (item) => !scannedItems.some((scanned) => scanned.barcode === item.barcode)
            )
                    ...
                    <div className="mt-5 grid grid-cols-1 md:grid-col-3 gap-4 xl:grid-cols-[repeat(auto-fill,minmax(184px,1fr))]">
                        {
                            items.map((v) => (
                                <ProductCard2 
                                    ...
                                    isSelected={selectedItems.some(item => item.barcode === v.barcode)}
                                />
                            ))
                        }
